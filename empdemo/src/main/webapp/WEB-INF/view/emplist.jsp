...
<div class="container">
    <div class="row head">
        <div class="col-sm-2">
            欢迎${user.username}
        <img src="${pageContext.request.contextPath}${user.headimg}" class="img-circle" width="20px" height="20px">
    </div>
    </div>
    <div class="row">
        <table class="table table-striped table-hover">
            ...
            <c:forEach items="${pageInfo.list}" var="emp" varStatus="index">
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-info update-btn" value="${emp.id}">修改</button>
                        <button type="button" class="btn btn-danger delete-btn" value="${emp.id}">删除</button>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <div class="col-sm-3">
            <nav aria-label="Page navigation">
                <ul class="pagination">
                    ...
                    <c:if test="${!pageInfo.isFirstPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/emp/emplist?pageNum=${pageInfo.prePage}"
                               aria-label="Previous">
                                <span aria-hidden="true">&laquo;</span>
                            </a>
                        </li>
                    ...
                    <c:if test="${(pageInfo.pages - pageInfo.pageNum) > 4}">
                        <c:forEach begin="${pageInfo.pageNum}" end="${pageInfo.pageNum + 4}" var="Num"
                            ...
                            <c:if test="${index.index == pageInfo.pageNum}">
                                <li class="active"><a
                                        href="${pageContext.request.contextPath}/emp/emplist?pageNum=${index.index}">${Num}</a>
                                </li>
                            </c:if>
                            <c:if test="${!(index.index == pageInfo.pageNum)}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/emp/emplist?pageNum=${index.index}">${Num}</a>
                                </li>
                            </c:if>
                        </c:forEach>
                    </c:if>
                    <c:if test="${!((pageInfo.pages - pageInfo.pageNum) > 4)}">
                        <c:forEach begin="${(pageInfo.pages - 4) > 0 ? (pageInfo.pages - 4):1}" end="${pageInfo.pages}"
                                   var="Num" varStatus="index">
                            <c:if test="${index.index == pageInfo.pageNum}">
                                <li class="active"><a
                                        href="${pageContext.request.contextPath}/emp/emplist?pageNum=${index.index}">${Num}</a>
                                </li>
                    ...
                    <c:if test="${!pageInfo.isLastPage}">
                        <li>
                            <a href="${pageContext.request.contextPath}/emp/emplist?pageNum=${pageInfo.nextPage}"
                               aria-label="Next">
                                <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
